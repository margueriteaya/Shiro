...
原始連結為 <${articleUrl}></span><br ><br >`
      ...
      await post().catch((err) => {
        console.error(err)
        toast.error('xLog PO失敗')

        throw err
      })

      toast.success('xLog PO成功')
      ...
      if (!nextPageId) {
        toast.error('無法擷取 Crossbell Note pageId 任務終止')
        return
      }
      ...
      if (!crossbellNoteData) {
        toast.error('無法擷取 Crossbell Note 任務終止')
        return
      }
      const {
        metadata,
